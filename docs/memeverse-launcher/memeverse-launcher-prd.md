...
Memeverse 需要一个统一的启动中枢，把 memecoin 注册、Genesis 募资、初始流动性创建、POL 分配、流动性解锁、费用分发和跨链治理收益接收串成一条明确的生命周期主链路。没有这个中枢，协议各模块虽然可以分别工作，但项目创建、资金归集、治理接入和退出路径会缺少统一状态语义。
...
- 项目发起方：通过注册中心和 registrar 创建新的 Memeverse，希望系统自动生成 memecoin、POL 以及后续治理/流动性接入点。
...
As a 项目发起方，我希望通过注册中心和 registrar 创建新的 Memeverse，以便 launcher 为后续募资、流动性和治理流程建立统一状态入口。
