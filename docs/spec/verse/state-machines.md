...
- `Locked -> Unlocked` 迁移在同一笔 `changeStage()` 交易内先完成 `POLSplitter.settle(...)` 与可选 `POLend.executeGlobalSettlement(...)`，再写入 hook 保护时间；hook-side public swap protection 在该写入后生效
- 不声明 settlement callback window 由 launcher-side transient gate 或已生效的公开 swap block 保护；进入 `Unlocked` 后，赎回可用性由阶段与各函数自身条件决定
