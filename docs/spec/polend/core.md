...
- 成功时 mint 杠杆 `uAsset`
- 成功时把真付部分（realInterest = `totalLeveragedInterest - totalCreditInterest`）的杠杆利息按全局 reserve 容量拆分为 settlement reserve 与 `protocolTreasury` 收入；credit 部分无 token 流入，跳过 reserve/treasury 拆分
...
兼容性：存储字段保留旧名 `leveragedInterestPaid` 承载 real 部分（与 `creditInterestPaid` 分栏）；对外 view `leveragedInterestPaid(verseId, user)` 返回 real+credit 合计，与 `getUserLeveragedDebt` 合计口径一致；存储层拆栏，view 层仍合计。real 部分用 `totalLeveragedInterest - totalCreditInterest` 差值推导仅在 finalize reserve/treasury 拆分时用。
...
`finalizeLeveragedGenesis` 必须通过统一内部 credit helper 把真付部分（realInterest）的杠杆利息注入全局 reserve。GenesisCredit 抵扣的 credit 利息没有对应 `uAsset` token 流入，不进入 reserve / treasury 拆分：

```text
realInterest = market.totalLeveragedInterest - market.totalCreditInterest
capacity = maxReserve - reserve
credited = min(realInterest, capacity)
treasuryInterest = realInterest - credited
reserve += credited
```

credited 部分进入全局 reserve；`treasuryInterest` 转入 `POLend.protocolTreasury`。该 credit 不 mint，不增加 debt，不创建任何用户债权，并且必须 emit `SettlementDustReservedFromInterest`（`SettlementDustReservedFromInterest.realInterest` 为 real 部分，且 `credited + treasuryInterest == realInterest`；`CreditBurned.totalCreditInterest` 承载 credit 部分，二者合起来对应 `market.totalLeveragedInterest`）。
