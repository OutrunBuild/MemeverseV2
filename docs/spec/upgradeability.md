...
| `MemeverseUniswapHook` | `ERC1967Proxy` + UUPS | `initialize(initialOwner, treasury_, dynamicFeeEngine_, lpTokenImplementation_, preorderSettlementExecutor_)` | `_authorizeUpgrade(...) => onlyOwner + poolManager mismatch guardrail` | `src/swap/MemeverseUniswapHook.sol::initialize`、`::_authorizeUpgrade`；getter 区 `::dynamicFeeEngine`、`::lpTokenImplementation`、`::preorderSettlementExecutor` |
