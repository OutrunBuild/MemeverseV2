...
| `src/swap/MemeverseSwapRouter.sol` | 用户路由入口（swap/liquidity/fee preview/bootstrap） | swap/liquidity 用户路径 permissionless；fee claim 仅提供 Router 只读 preview helper，执行入口在 Hook core；`createPoolAndAddLiquidity(...)` 为 launcher-only bootstrap；构造固定 `hook/permit2` 绑定 | 构造部署，不走 proxy | [docs/spec/protocol.md](spec/protocol.md); [docs/spec/verse/state-machines.md](spec/verse/state-machines.md); `src/swap/MemeverseSwapRouter.sol:71-77`, `:89-92`, `:137-145`, `:174-240`, `:474-486`; src/swap/MemeverseUniswapHook.sol:614-620 | 已实现 |
| `src/swap/MemeverseDynamicFeeEngine.sol` | 动态费率引擎：launch fee 衰减、dynamic fee、per-pool override fee 管理 | authorizedHook 写入 fee state；fee config / override 配置 onlyOwner；`authorizedHook` 首次部署后不可变更 | ERC1967Proxy + UUPS；implementation constructor 固定 poolManager 并锁定 initializers；proxy 通过 `initialize(initialOwner, authorizedHook_)` 初始化，`authorizedHook_` 绑定唯一 Hook proxy；owner 授权升级并检查 poolManager 不变 | `src/swap/MemeverseDynamicFeeEngine.sol:85`, `:103-113` | 已实现 |
| `src/swap/MemeverseUniswapHook.sol` | fee engine 集成、LP 记账、hook callbacks、显式 launch settlement 执行 | 核心 API 对外开放；fee/treasury/launcher 配置 onlyOwner；launch settlement 与 pool-level 公开 swap 恢复时间写入仅 launcher；公开 swap 保护在 `beforeSwap` 执行 | ERC1967Proxy + UUPS；implementation constructor 仅固定 BaseHook/poolManager 并锁定 initializers；proxy initializer 绑定并校验 dynamicFeeEngine；proxy 通过 `initialize(initialOwner, treasury_, dynamicFeeEngine_)` 初始化，校验 engine 的 `authorizedHook` 与 `owner` 均指向本 hook proxy；owner 授权升级并检查 poolManager 不变；部署顺序为 engine impl → engine proxy → hook impl → hook proxy | [docs/spec/protocol.md](spec/protocol.md); [docs/spec/verse/state-machines.md](spec/verse/state-machines.md); `src/swap/MemeverseUniswapHook.sol:177`, `:309-377`, `:440`, `:510`, `:550`, `:565-627`, `:1031-1046` | 已实现 |
| `src/token/Memecoin.sol` + `src/token/MemePol.sol` | OFT memecoin/POL 资产层 | launcher 控制 mint 与 poolId；burn 按持币/授权路径开放 | clone + `initialize` 一次性 | [docs/spec/verse/accounting.md](spec/verse/accounting.md); [docs/spec/access-control.md](spec/access-control.md); `src/token/Memecoin.sol:24-43`; `src/token/MemePol.sol:37-66`; `src/common/access/Initializable.sol:31-41` | 已实现 |
| `src/yield/MemecoinYieldVault.sol` | memecoin yield vault + ERC20Votes share | 无 owner 门禁型业务入口（deposit/yield/redeem）；由流程和资产校验约束 | clone + `initialize` 一次性 | [docs/spec/verse/accounting.md](spec/verse/accounting.md); [docs/spec/access-control.md](spec/access-control.md); `src/yield/MemecoinYieldVault.sol:37-50`, `:86`, `:120`, `:132`, `:146` | 已实现 |
| `src/governance/MemecoinDaoGovernorUpgradeable.sol` | DAO Governor + treasury + proposal/vote 扩展 | treasury 支出/升级仅治理执行 | UUPS + initializer | [docs/spec/verse/accounting.md](spec/verse/accounting.md); `src/governance/MemecoinDaoGovernorUpgradeable.sol:76-92`, `:221`, `:252` | 已实现 |
...
| `src/verse/YieldDispatcher.sol` | OFT compose 收益分发（governor / yieldVault / burn） | `lzCompose` 仅 endpoint 或 launcher | 构造部署，不走 proxy | [docs/spec/protocol.md](spec/protocol.md); [docs/spec/access-control.md](spec/access-control.md); `src/verse/YieldDispatcher.sol:39-47`, `:63-79` | 已实现 |
