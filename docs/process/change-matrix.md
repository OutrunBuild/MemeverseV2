...
- `docs/task-briefs/**`：只存放 `Task Brief`；命中需要落盘 brief 的流程时，路径应回溯到本目录；仓库默认本地忽略 `docs/task-briefs/*.md`，但保留 `docs/task-briefs/README.md`
...
命中 `AGENTS.md`、`README.md`、`docs/process/**`、`docs/reviews/TEMPLATE.md`、`docs/reviews/README.md`、`docs/task-briefs/**`、`docs/agent-reports/**`、`.github/pull_request_template.md`、`.codex/**` 时：

- 默认写入角色：`process-implementer`
- 默认验证角色：`verifier`
- 评审顺序遵循 Harness 契约：`main-orchestrator` -> `verifier`
- 至少执行：`npm run docs:check`
- 命中 `docs/task-briefs/**` 或 `docs/agent-reports/**` 时，仍受本地工件三目录约束：`docs/plans/` 不回退承载 brief/report，且目录内 `README.md` 之外的 `*.md` 默认按 local-only 管理
