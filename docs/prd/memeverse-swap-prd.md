...
- 或直接接入 Hook 的低层能力
- 在 anti-snipe 窗口内 permissionless 调用 `requestSwapAttemptWithQuote(...)`
...
`requestSwapAttemptWithQuote(...)` 本身不需要 Router 白名单。
...
尽管 `requestSwapAttemptWithQuote(...)` 是 permissionless：
...
- Witness 绑定必须遵循 Permit2 标准 EIP-712 形式：`<WitnessType> witness)<WitnessType>(...)TokenPermissions(...)`，并使用 typed witness hash（包含 witness struct typehash）
- Permit2 签名中的 spender 固定为 Router 地址；Router 侧强制 `transferDetails.to == address(this)`
